...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    m_statusCenter = new QLabel(tr("Total: 0 min"), this);
    ...
    if (APPLICATION->accounts()->count() == 0) {
        QTimer::singleShot(0, this, [this] {
            ChooseOfflineNameDialog dialog(QString(), this);
            if (dialog.exec() == QDialog::Accepted) {
                if (auto account = MinecraftAccount::createOffline(dialog.getUsername())) {
                    account->login()->start();
...
{
    DesktopServices::openUrl(QUrl("https://racked.ru"));
}
...
void MainWindow::updateStatusCenter()
{
    ...
    m_statusCenter->setText(tr("Total: %1 min").arg(timePlayed / 60));
}
