...
QString Time::relativePast(const QDateTime& past)
{
    qint64 secs = past.secsTo(QDateTime::currentDateTime());
    if (secs < 0)
        secs = 0;
    ...
    if (secs < 86400 * 60)
        return QObject::tr("%1w ago").arg(secs / (86400 * 7));
    if (secs < 86400 * 365)
        return QObject::tr("%1mo ago").arg(secs / (86400 * 30));
    return QObject::tr("%1y ago").arg(secs / (86400 * 365));
}
