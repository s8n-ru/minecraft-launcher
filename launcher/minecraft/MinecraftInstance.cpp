...
QString MinecraftInstance::getStatusbarDescription()
{
    ...
    if (m_settings->get("ShowGameTime").toBool() && lastTimePlayed() > 0 && lastLaunch() > 0) {
        QDateTime lastLaunchTime = QDateTime::fromMSecsSinceEpoch(lastLaunch());
        description.append(tr("last played %1 — %2 min")
                               .arg(Time::relativePast(lastLaunchTime))
                               .arg(lastTimePlayed() / 60));
    }

    if (hasCrashed()) {
        if (!description.isEmpty())
            description.append(QStringLiteral(" — "));
        description.append(tr("crashed"));
    }
    return description;
}
